...
class PostQuerySet(models.QuerySet):
    ...
    def popular(self):
        return self.annotate(
            post_count=Count('likes', distinct=True)).order_by('-post_count')

    def fresh(self):
        return self.annotate(
            post_count=Count('likes', distinct=True)).order_by('-published_at')
    ...
    def fetch_with_tags_counts(self):
...
class Comment(models.Model):
    post = models.ForeignKey('Post', on_delete=models.CASCADE, related_name='comments', verbose_name='Пост, к которому написан')
